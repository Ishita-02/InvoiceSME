...
const InvestmentModal = ({ invoice, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const amountInWei = web3Service.toWei(amount);
            
            await web3Service.executeInvestment(
                account,        
                invoice.seller, 
                invoice.id,     
                amountInWei
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
            <div className="bg-white rounded-lg p-8 shadow-2xl w-full max-w-md m-4">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-[#1E4D43]">Invest in Invoice #{invoice.id}</h2>
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-800">&times;</button>
                </div>

                <div className="space-y-2 mb-6 text-sm">
                    <p><strong>Funding Goal:</strong> {invoice.discountValue} PYUSD</p>
                    <p><strong>Seller:</strong> <span className="text-xs break-all">{invoice.seller}</span></p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            step="0.01"
                            min="0"
                            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[#1E4D43]"
                        />
                    </div>
